-- Add qualified_at column to prospects table
-- This column tracks when a prospect becomes "qualified" (i.e., a real user signed up)
-- Separates lead qualification from report generation status

ALTER TABLE public.prospects ADD COLUMN IF NOT EXISTS qualified_at TIMESTAMPTZ;

-- Update the handle_new_user trigger to set qualified_at when linking outbound prospects
...
BEGIN
    ...
    IF org_id IS NOT NULL THEN
        ...
        IF EXISTS (SELECT 1 FROM public.prospects WHERE company_domain = user_domain) THEN
             ...
             -- Set qualified_at to mark this prospect as qualified (user signed up)
